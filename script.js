...
async function generateResponse() {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            showToast('API Error: ' + (response.status === 400 ? 'Invalid request - check model name' : errorText), 'error');
...
// Auto-hide advertisement placeholder when real ads are loaded
function checkAndHideAdPlaceholder() {
    ...
    // Check if there are any real ad elements loaded (common ad network patterns)
    const hasRealAds = adContainer.querySelector('iframe') || 
                       adContainer.querySelector('ins') || 
                       adContainer.querySelector('[data-ad-client]') ||
                       adContainer.querySelector('[data-ad-slot]') ||
                       adContainer.children.length > 1; // More than just the placeholder
...
window.addEventListener('load', checkAndHideAdPlaceholder);

// Check for ads periodically (every 2 seconds) in case ads load dynamically
setInterval(checkAndHideAdPlaceholder, 2000); 