...
const LaurixyVoice = (() => {
    ...
    function speak(text, force = false) {
        ...
        // Internal function to actually perform the speak
        function doSpeak() {
            ...
            // Android WebView bug workaround: resume synthesis if paused
            window.speechSynthesis.resume();
            window.speechSynthesis.speak(utterance);

            // Android WebView bug: synthesis can pause randomly after ~15s 
        ...
        // Voices may not be loaded yet (common in Android WebView)
        const voices = window.speechSynthesis.getVoices();
        if (voices.length > 0) {
            doSpeak();
        } else {
            ...
            // Fallback: if voices don't load within 500ms, try anyway
            setTimeout(() => {
                if (window.speechSynthesis.getVoices().length === 0) {
                    // No voices available at all — try speaking without a voice set
                    doSpeak();
                }
            }, 500);
        }
